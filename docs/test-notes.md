# Test Notes

Use this file to capture things you want to add, adjust, or fix while testing the app.

## Notes
- I want to be able to import line ups as they drop so all users have easy access to them.
- 

## Fixes Needed
- Top of the application intersects with the top bar used for notifcations, time, and the camera lens.
- Lineup component has weird symbol tabs on top that are clickable, the plus sign does not work so unable to add artists.
- The back icon does not work. iOs swipe feature works.
- Clicking add flight just brings up a cancel or save flight button. There is no options to add details. [I can provide more details for this when it is time to fix this]. Adding a vehichle does the same.
## Follow-Up
- Why is outfit voting in travel plans?
-  Travel plans map - does it allow zooming in with touch gestures?
-  Need to add additional features with the map, such as multiple pin drops with lables, option to find intersecting routes between cars for easy meet ups. Gas calculator. I will look more into this and get more details.
- Unqiue usernames for easy look up and to find friends and add them as a contact [Can implement features related to this later]
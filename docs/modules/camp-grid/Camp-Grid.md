...
Land on camp grid → DimensionModal appears
Select Festival (Electric Forest)
Select Ticket (GA)
See 20×20 grid auto-rendered
ItemLibrarySidebar visible on right
Tap "Add Tent" → tent appears in center of grid
Drag to position, snaps to 1ft grid
Tap item to select; trash icon to delete
Nav away → auto-save to SQLite
Returning to Camp Grid
Load from SQLite
All previous items + grid size displayed
Continue editing
Technical Approach
Grid Rendering
...
Create festivalPresets.ts
Create SQLite db.ts + migration file
Set up expo-sqlite + type definitions
Stage 2: UI Shells
Create CampGridScene (empty)
Create ItemLibrarySidebar (empty)
Create DimensionModal (empty)
Create camp-grid.tsx route
Stage 3: Core Grid Logic
Implement useCampGridDB hook (CRUD)
Implement grid rendering in CampGridScene
Implement PanResponder for drag/drop
Implement grid snapping logic
Stage 4: UI Polish
ItemCard component (draggable item visuals)
ItemLibrarySidebar interaction (add/remove)
DimensionModal flow (festival → ticket → confirm)
Orientation lock + responsive sizing
Stage 5: Integration
Wire DimensionModal to first visit
Wire ItemLibrarySidebar to add items
Wire drag to move items, update SQLite
...
Open app → create trip → navigate to camp grid screen
Land on empty grid → DimensionModal appears
Select Electric Forest → GA → confirm 20×20 grid
Grid renders with 1ft cells
Tap "Add Tent" → 8×8 tent appears at center
Drag tent → moves smoothly, snaps to 1ft grid on release
Add car → 10×6 item placed
Delete tent via trash icon
Navigate away → toast "Saved to device"
Return to camp grid → items restored from SQLite
Landscape rotation works smoothly; portrait shows orientation warning
